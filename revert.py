...
with open('c:/Users/PANVI KAUSHIK/Desktop/Health123/App.tsx', 'r', encoding='utf-8') as f:
    content = f.read()

# Remove useGoogleLogin import
content = re.sub(r"import\s+\{\s*useGoogleLogin\s*\}\s*from\s*'@react-oauth/google';\s*\n", "", content)

new_login = """const LoginView = ({ role, onLoginSuccess, onBack, backText = "← Back to Portal Selection" }: { role: 'patient' | 'doctor', onLoginSuccess: (email: string) => void, onBack: () => void, backText?: string }) => {
...
old_login_pattern = re.compile(r'const LoginView = \(\{ role, onLoginSuccess.*?\}\);\n\};', re.DOTALL)
content = old_login_pattern.sub(new_login, content)

with open('c:/Users/PANVI KAUSHIK/Desktop/Health123/App.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
print('Done!')
